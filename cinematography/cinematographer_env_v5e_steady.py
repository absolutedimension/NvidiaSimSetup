"""Isaac Lab environment v5e: STEADY cinematographer drone — improved stability + smooth transitions.

v5e builds on v5d's successful damping + EMA foundation, fixing two problems:
  1. Training collapse at epoch ~300 (fixed via conservative PPO hyperparams)
  2. Jerky transitions between hold positions (fixed via transition smoothness reward)

Changes from v5d:
  - ACTIVATED hold_bonus in reward (was computed but unused): rewards sustained holds
  - ACTIVATED action_smoothness in reward: penalizes jerky action changes
  - Added jerk penalty: penalizes rate-of-change of acceleration for ultra-smooth transitions
  - Rebalanced weights: stillness 0.45, look_at 0.15, distance 0.08, hold_bonus 0.12,
                         action_smooth 0.10, jerk_penalty 0.05, safety 0.05
  - All physics unchanged from v5d: damping, EMA alpha=0.15, moment_scale=0.01
"""
...
from .cinematographer_env_cfg_v5e_steady import CinematographerEnvCfgV5eSteady  # noqa: F401


class CinematographerEnvV5eSteady(DirectRLEnv):
    cfg: CinematographerEnvCfgV5eSteady

    def __init__(self, cfg: CinematographerEnvCfgV5eSteady, render_mode: str | None = None, **kwargs):
        ...
        # Action EMA filter (unchanged from v5d)
        ...
        # -- Physics state for jerk computation --
        ...
        # -- History buffers (for obs compatibility) --
    ...
    # -- Observations: 20-dim (same layout as v5d) ---------------------------
    ...
    # -- Rewards: v5e — stillness + smooth transitions -----------------------
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        jerk = (accel - self._prev_accel) / sim_dt  # v5e: jerk for smooth transitions
        ...
        # =====================================================================
        # v5e: 6 ACTIVE REWARD TERMS (v5d had 3 + safety)
        # Keeps the stillness foundation, adds smoothness for transitions
        # =====================================================================

        # -- r_stillness: GAUSSIAN peaked at zero velocity (from v5d) --
        r_stillness = torch.exp(-drone_speed.pow(2) * 50.0) * torch.exp(-ang_speed.pow(2) * 20.0)

        # -- r_look_at: camera aims at dancer (from v5d) --
        cos_angle = (to_dancer * fwd_norm).sum(dim=-1).clamp(-1, 1)
        look_angle = cos_angle.acos()
        r_look_at = 1.0 - (look_angle / math.radians(15)).tanh()

        # -- r_distance: maintain ideal filming distance (from v5d) --
        dist_err = dist - self._dist_ideal
        r_distance = 1.0 - (dist_err.abs() / self._dist_margin).tanh()

        # -- r_safety: don't crash into dancer (from v5d) --
        violation = (dist < self._safety_radius).float()
        r_safety = violation * (-5.0)

        # -- r_hold_bonus: reward SUSTAINED holds (NEW in v5e reward sum) --
        # Linearly ramps from 0 to 1 over 3 seconds of holding still
        # This teaches the policy to STAY still, not just momentarily stop
        r_hold_bonus = (self._stillness_time > 0.5).float() * (self._stillness_time / 3.0).clamp(0, 1)

        # -- r_action_smooth: penalize jerky action changes (NEW in v5e reward sum) --
        # Small action deltas = smooth. Uses both first and second order differences.
        action_delta = (self._actions - self._prev_actions).norm(dim=-1)
        action_delta2 = (self._actions - 2.0 * self._prev_actions + self._prev_prev_actions).norm(dim=-1)
        r_action_smooth = torch.exp(-action_delta * 5.0) * torch.exp(-action_delta2 * 3.0)

        # -- r_jerk_penalty: ultra-smooth transitions via jerk minimization (NEW in v5e) --
        # Jerk = d(acceleration)/dt. Low jerk = smooth, professional camera motion.
        # This specifically makes the transitions between hold positions buttery.
        jerk_magnitude = jerk.norm(dim=-1)
        r_jerk_penalty = torch.exp(-jerk_magnitude * 0.02)  # gentle — don't prevent all movement

        # Update hold_quality for obs
        self._hold_quality = r_look_at.detach() * (1.0 - drone_speed.detach().clamp(0, 1))

        # For logging: composition is same as look_at, angle_variety unused
        r_composition = r_look_at
        r_angle_variety = torch.zeros_like(r_stillness)

        # -- Weighted sum --
        # v5e balance: stillness still dominant, but smooth transitions rewarded
        total = (
            0.45 * r_stillness +        # core: be still (was 0.65 in v5d)
            0.15 * r_look_at +           # face the dancer (was 0.20)
            0.08 * r_distance +           # right distance (was 0.10)
            0.12 * r_hold_bonus +         # sustain holds (was 0.00 — NOT USED in v5d!)
            0.10 * r_action_smooth +      # smooth actions (was 0.00 — NOT USED in v5d!)
            0.05 * r_jerk_penalty +       # smooth transitions (NEW)
            0.05 * r_safety               # don't crash (unchanged)
