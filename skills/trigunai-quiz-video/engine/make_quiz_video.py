...
# ---------- portable font resolution (macOS + Linux/EC2) ----------
def _pick(cands):
    for p, i in cands:
        if os.path.exists(p):
            return p, i
    raise FileNotFoundError("no font found among: " + ", ".join(p for p, _ in cands))

# Latin bold (headings, brand, numbers, timer digit)
F_BLD, F_BLD_I = _pick([
...
# Latin w/ superscripts (⁻¹ ²) + symbols (✓) for EN option/formula text
F_SUP, F_SUP_I = _pick([
    ("/System/Library/Fonts/Supplemental/Arial Unicode.ttf", 0),
    ("/usr/share/fonts/truetype/noto/NotoSans-Regular.ttf", 0),
    ("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 0),
])
# Devanagari bold (Hindi headings/questions)
DEVA_BLD, DEVA_BLD_I = _pick([
    ("/System/Library/Fonts/Kohinoor.ttc", 3),
    ("/usr/share/fonts/truetype/noto/NotoSansDevanagari-Bold.ttf", 0),
    (os.path.expanduser("~/.local/share/fonts/NotoSansDevanagari-Bold.ttf"), 0),
])
# Devanagari regular (Hindi body/options/explanation)
DEVA_REG, DEVA_REG_I = _pick([
    ("/System/Library/Fonts/Kohinoor.ttc", 0),
    ("/usr/share/fonts/truetype/noto/NotoSansDevanagari-Regular.ttf", 0),
    (os.path.expanduser("~/.local/share/fonts/NotoSansDevanagari-Regular.ttf"), 0),
    ("/usr/share/fonts/truetype/noto/NotoSansDevanagari-Bold.ttf", 0),  # last resort
    (os.path.expanduser("~/.local/share/fonts/NotoSansDevanagari-Bold.ttf"), 0),
])
...
def fh(sz):   # heading / bold content text
    return font(F_BLD, sz, F_BLD_I) if LANG == "en" else font(DEVA_BLD, sz, DEVA_BLD_I)
def fo(sz):   # body / option content text (superscript-capable font for EN formulas)
    return font(F_SUP, sz, F_SUP_I) if LANG == "en" else font(DEVA_REG, sz, DEVA_REG_I)

# ---- mixed-script text (Devanagari runs → Deva font, Latin/digits/punct → Latin font) ----
def _cf(is_deva, bold, sz):
    if is_deva: return font(DEVA_BLD, sz, DEVA_BLD_I) if bold else font(DEVA_REG, sz, DEVA_REG_I)
    return font(F_BLD, sz, F_BLD_I) if bold else font(F_SUP, sz, F_SUP_I)
def _runs(text):
    out, cur, cd = [], "", None
    for ch in text:
        d = 0x0900 <= ord(ch) <= 0x097F          # Devanagari block (letters + matras)
        if cd is None: cd, cur = d, ch
        elif d == cd: cur += ch
        else: out.append((cur, cd)); cur, cd = ch, d
    if cur: out.append((cur, cd))
    return out
def mlen(d, text, sz, bold):
    return sum(d.textlength(s, font=_cf(isd, bold, sz)) for s, isd in _runs(text))
def mtext(d, xy, text, sz, bold, fill):
    x, y = xy
    for s, isd in _runs(text):
        f = _cf(isd, bold, sz); d.text((x, y), s, font=f, fill=fill)
        x += d.textlength(s, font=f)
def draw_check(d, cx, cy, s, col, w=8):   # vector tick — portable, no font glyph needed
    d.line([(cx - s*0.55, cy + s*0.05), (cx - s*0.15, cy + s*0.5), (cx + s*0.6, cy - s*0.5)],
...
def wrap(draw, text, sz, bold, max_w):
    words, lines, cur = text.split(), [], ""
    for w in words:
        t = (cur + " " + w).strip()
        if mlen(draw, t, sz, bold) <= max_w: cur = t
        else:
...
def header(d, sub, chap):
    ...
    mtext(d, (134, 138), f"{sub}  ·  {chap}", 33, False, MUTED)
...
def qbadge(d, n, total, label):
    ...
    mtext(d, (272, 256), label, 36, True, GOLD_DK)
...
def timer_ring(d, cx, cy, r, frac, secs):
    ...
    lbl = UI["seconds"]; lw = mlen(d, lbl, 32, False)
    mtext(d, (cx - lw / 2, cy + 62), lbl, 32, False, MUTED)

def cta_bar(d, text):
    soft_card(d, [70, H - 196, W - 70, H - 96], 30, GOLD, GOLD_DK, 0, shadow=True)
    tw = mlen(d, text, 46, True)
    mtext(d, (W / 2 - tw / 2, H - 174), text, 46, True, (255, 248, 240))
...
def celebrate(d, origin, local, dur=1.8):
    ...
    if local < 1.4:
        ...
        d.rounded_rectangle([bx, by, bx+bw, by+bh], radius=int(bh/2),
                            fill=(47,169,104,al))
        if sc > 0.5:
            word = UI["correct"]; ww = mlen(d, word, 52, True); chk = 44
            x0 = ox - (chk + 14 + ww) / 2
            draw_check(d, x0 + chk*0.5, oy - 4, chk*0.5, (255,255,255,al), 7)
            mtext(d, (x0 + chk + 14, oy - 34), word, 52, True, (255,255,255,al))
...
def render_mcq(d, q, phase, local, seg_dur, TIMER, sub, chap, qi, nq):
    ...
    # question
    y=352
    for ln in wrap(d, q["q"], 60, True, W-150):
        mtext(d,(70,y), ln, 60, True, INK); y+=76
    y+=16
    ...
    for i,opt in enumerate(q["options"]):
        ...
        ol=wrap(d,opt,50,False,w-200)
        ty=oy+bh//2-(len(ol)*58)//2
        for ln in ol: mtext(d,(x+140,ty),ln,50,False,INK); ty+=58
        if correct:
            draw_check(d, x+w-72, oy+bh//2, 32, GREEN_DK, 9)
    ylast = y+4*(bh+gap)
    if phase=="count":
        ...
        txt=UI["timesup"]; tw=mlen(d,txt,104,True)
        mtext(d,(W/2-tw/2,H-400),txt,104,True,(200,70,45) if blink else GOLD_BRT)
    elif phase=="reveal":
        ey=ylast+20
        el=wrap(d,UI["why"]+q["explain"],42,False,W-200)
        eh=52+len(el)*54
        soft_card(d,[70,ey,W-70,ey+eh],22, GREEN_BG, GREEN,2)
        yy=ey+26
        for ln in el: mtext(d,(104,yy),ln,42,False,GREEN_DK); yy+=54
        # celebration anchored on correct option
        cyc = y+q["answer"]*(bh+gap)+bh//2
        if local < 1.9: celebrate(d,(W//2,cyc),local)
    elif phase=="qintro":
        mtext(d,(70,H-250),UI["lockin"],40,False,MUTED)

def render_match(d, q, phase, local, seg_dur, TIMER, sub, chap, qi, nq):
    header(d, sub, chap); qbadge(d, qi+1, f"of {nq}", UI["match"])
    y=330
    for ln in wrap(d, q["prompt"], 54, True, W-150):
        mtext(d,(70,y),ln,54,True,INK); y+=66
    y+=12
    ...
    for i,item in enumerate(left):
        ...
        il=wrap(d,item,44,False,colw-120)
        ty=oy+rowh//2-(len(il)*50)//2
        for ln in il: mtext(d,(lx0+112,ty),ln,44,False,INK); ty+=50
    # right cards (letters)
    for j,item in enumerate(right):
        ...
        il=wrap(d,item,44,False,colw-120)
        ty=oy+rowh//2-(len(il)*50)//2
        for ln in il: mtext(d,(rx0+24,ty),ln,44,False,INK); ty+=50
    ylast=ys[-1]+rowh
    if phase=="count":
        secs=min(TIMER,max(1,int(math.ceil(seg_dur-local-1e-6))))
        ...
        blink=int(local*8)%2==0
        txt=UI["timesup"]; tw=mlen(d,txt,96,True)
        mtext(d,(W/2-tw/2,H-360),txt,96,True,(200,70,45) if blink else GOLD_BRT)
    elif phase=="qintro":
        mtext(d,(70,H-250),UI["pair"],40,False,MUTED)
    elif phase=="reveal":
...
def build(content_path, out_path):
    ...
    for fi in range(TF):
        ...
        if k=="intro":
            header(d,sub,chap)
            mtext(d,(70,540),sub.upper(),92,True,GOLD)
            yy=660
            for ln in wrap(d,chap,118,True,W-140):
                mtext(d,(70,yy),ln,118,True,INK); yy+=138
            mtext(d,(70,yy+18),data.get("topic_line",""),44,False,MUTED)
            a=int(150+95*abs(math.sin(local*3)))
            mtext(d,(70,1190),UI["beat"],62,True,(194,89,31,a))
            cta_bar(d,cta)
        elif k=="outro":
            header(d,sub,chap)
            mtext(d,(70,600),UI["nice"],100,True,INK)
            yy=760
            for ln in wrap(d,data.get("outro_voice",""),56,False,W-150):
                mtext(d,(70,yy),ln,56,False,MUTED); yy+=72
