...
└─────────────────────────────┘         └─────────────────────────────┘
         │                                        │
         │  ┌─────────────────────────────┐       │
         └──│  LOWER BODY PHYSICS AGENT   │───────┘
            │  (Mac + EC2, same infra)    │  Quest upper body
            │                             │  → full body with
            │  Isaac Lab AMP + PhysX      │    natural legs
            │  Pinned upper body env      │
            │  lower_body_physics/ dir    │
            └─────────────────────────────┘
```
...
| Agent | Skill | Purpose |
|---|---|---|
| Lower Body Physics | `trigunai-lower-body-physics` | Quest upper body → full body via Isaac AMP. Phases LB1→LB7. **LB5 is SUBJECTIVE.** |
| CEO | `trigunai-ceo` | Master Founding OS v3.0. Holds ground truth. Makes gate decisions. Anti-inflation guard. |
| Project Hub | `trigunai-project-hub` | Central nervous system. CEO briefing, cross-agent feedback, artifact registry. |

All agents read/write `project_hub/` for coordination. CEO reads `project_hub/CEO_BRIEFING.md`
as the single status view. Agents write feedback to `project_hub/feedback/` for async communication.
...
Video rendering: Blender EEVEE preferred (0.33s/frame, 18x faster than OVRTX). See VIDEO_RENDERING.md
OVRTX: localhost:8001 (LEGACY — only for MDL materials. 6s/frame, 6 min cold start)
...
Blender: /opt/blender45 (symlink: blender45) — GPU rendering via EEVEE or Cycles+OptiX
