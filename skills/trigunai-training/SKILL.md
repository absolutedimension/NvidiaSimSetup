...
### 3. Video rendering → MP4

**Read `VIDEO_RENDERING.md` (repo root) for the full reference.** Summary:

**PREFERRED: Blender GPU rendering** (EEVEE or Cycles+OptiX). 2.5x faster than OVRTX,
no HTTP overhead, renders directly on the A10G GPU.

```bash
# Fast preview (~2.5 s/frame at 1280x720)
...
# RTX ray-traced (~3 s/frame, best quality)
blender45 --background --python /home/ubuntu/render_blender_drone_pov.py -- \
  --dancer ... --trajectory ... --out ... --engine cycles --samples 64
```

**LEGACY: OVRTX API** (port 8001). Use only when you need NVIDIA MDL material shading.
- Base64-encode USDA into a data URI → POST to `/render` → batch 50 frames max
- ~6 s/frame, 75 min for 25s video

**Key scripts:**
- `cinematography/render_blender_drone_pov.py` — Blender GPU renderer (preferred)
- `cinematography/render_dancer_mp4.py` — OVRTX legacy renderer
...
2. Check `project_hub/feedback/*_to_training*.md` for unread messages from other agents
...
2. Write feedback files to `project_hub/feedback/` for other agents if you produced deliverables
...
5. If a gate was reached, add to `project_hub/GATE_LOG.md`
