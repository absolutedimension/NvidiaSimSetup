...
BPSC Prelims questions are ALL multiple-choice with exactly 4 options labelled (A)(B)(C)(D)
(sometimes (1)(2)(3)(4)). Capture each as qtype "MCQ_single" with 4 options.
...
- Transcribe the stem and all 4 options EXACTLY as printed. Options: drop the "(A)" label, keep
...
def main():
    ...
    for i, img in enumerate(pdf_pages_to_images(args.pdf, args.dpi), 1):
        for q in run_page(model, processor, img, max_new_tokens=args.max_new_tokens):
            ...
            labels = [l for l in "ABCD" if str(opts.get(l, "")).strip()]
            by_number[str(n)] = {
