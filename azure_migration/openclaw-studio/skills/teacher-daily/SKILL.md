...
Kit lives at `~/teacher_gtm/` on the box: `15DAY_PLAN.md`, `DAY{N}_BRIEF.md`, `progress.py` (+ `progress.json`), `03_CONVERSATION_LOG.md`, `02_CONVERSATION_SCRIPT.md`, `06_SOURCING_CHANNELS.md`.
...
3. **Source 6 fresh leads** (agent-digs-you-verify) via **Azure Maps** (the box's web/lead tool):
   ...
   ```
   Returns real coaching-centre `name | phone | address`. **Phones are usually blank** (dataset gap) → deliver `{name, area, address}` as **candidates for Deepak or the field rep to verify the number on Justdial before dialling** (never claim a number is verified). Pick 6 fresh ones not already in `03_CONVERSATION_LOG.md`. (General web search from this box is blocked by anti-bot walls — use Azure Maps for local business leads; the OpenClaw `browser` is the fallback for a specific page.)
4. **The field rep's list:** pick their 5 numbers for the day from the queue + yesterday's feedback.
5. **Send the Telegram brief** to Deepak (8478318652), tight:
   ...
   > Targets: 2 conversations (you) + 1–2 (the field rep). Reply outcomes as they happen — I'll log them.
