...
| **M2 — Our singers** ✅ DONE 2026-06-15 | **Trigun-Maya (F) + Trigun-Ravi (M)** working. **Pivot from RVC → seed-vc:** this box runs torch 2.12 / py3.12, where RVC's `fairseq`/HuBERT deps break — so we used **seed-vc zero-shot singing voice conversion** (NO training): a fixed reference clip per singer = a permanent identity. `singerize.py` (on box, `m2_venv`): song → **Demucs** split → **seed-vc** convert vocal to singer (`--f0-condition`, RTF 0.43) → remix → master → MP3. Singer refs at `/home/ubuntu/singers/{maya,ravi}_ref.wav` (seeded from F5-TTS `female_calm`/`male_calm`). Validated: Maya on the ghazal, Ravi on the pop track. Add a singer by dropping a clean ref clip in `singers/`. | Same voice across different songs ✅ |
