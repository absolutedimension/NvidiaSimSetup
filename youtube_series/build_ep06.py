...
HOME="/home/ubuntu"; B=f"{HOME}/youtube_series/ep06_manim_build"; BUILD=f"{HOME}/youtube_series/ep06_build"
OUT=f"{HOME}/youtube_series/ep06_FINAL.mp4"; N=12

print("== STEP 1: render scenes (boomerang bg) ==",flush=True)
subprocess.run(["python3","render_ep06_manim.py"],cwd=HOME)
...
subprocess.run(["python3","focus_audio.py","--dur",str(int(float(d))),"--beat","12","--out","/tmp/bed_ep06_en.wav"],cwd=HOME)
FOC=f"{HOME}/youtube_series/ep06_FINAL_focus.mp4"
subprocess.run(["python3","focus_mix.py",OUT,"/tmp/bed_ep06_en.wav",FOC],cwd=HOME)
