...
HOME="/home/ubuntu"; B=f"{HOME}/youtube_series/ep06_manim_hi_build"; BUILD=f"{HOME}/youtube_series/ep06_hi_build"
OUT=f"{HOME}/youtube_series/ep06_hi_FINAL.mp4"; N=12

print("== STEP 1: render Hindi scenes (boomerang bg) ==",flush=True)
subprocess.run(["python3","render_ep06_manim_hi.py"],cwd=HOME)
...
subprocess.run(["python3","focus_audio.py","--dur",str(int(float(d))),"--beat","12","--out","/tmp/bed_ep06_hi.wav"],cwd=HOME)
FOC=f"{HOME}/youtube_series/ep06_hi_FINAL_focus.mp4"
subprocess.run(["python3","focus_mix.py",OUT,"/tmp/bed_ep06_hi.wav",FOC],cwd=HOME)
