#!/usr/bin/env python3
"""HINDI driver: ep01_manim_hi.py (Devanagari text) + SAME background clips + SAME logo finish.
Usage: python3 render_ep01_manim_hi.py [all|6,7]"""
import os, sys, glob, subprocess
HOME="/home/ubuntu"
BUILD=f"{HOME}/youtube_series/ep01_hi_build"          # Hindi audio
CLIPDIR=f"{HOME}/youtube_series/clips"                # SAME bg clips (reused)
WORK=f"{HOME}/youtube_series/ep01_manim_hi_build"; os.makedirs(WORK, exist_ok=True)
OUT=f"{HOME}/youtube_series/ep01_hi_v1.mp4"
...
for i in todo:
    ...
    r=subprocess.run(["python3","-m","manim","-r","1920,1080","--fps","30","--transparent",
                      "--disable_caching","-o",sc,"ep01_manim_hi.py",sc], cwd=HOME, env=env, capture_output=True, text=True)
    movs=glob.glob(f"{HOME}/media/videos/ep01_manim_hi/**/{sc}.mov", recursive=True)
    if not movs:
...
print(f"\n(have {len(clips)}/10 hi scene clips)", flush=True)
