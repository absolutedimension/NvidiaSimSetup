#!/usr/bin/env python3
"""Full HINDI Episode 2: Hindi Mode-C scenes (boomerang bg) -> Hindi captions -> concat -> focus bed."""
import subprocess, os
HOME="/home/ubuntu"; B=f"{HOME}/youtube_series/ep02_manim_hi_build"; BUILD=f"{HOME}/youtube_series/ep02_hi_build"
OUT=f"{HOME}/youtube_series/ep02_hi_FINAL.mp4"; N=13

print("== STEP 1: render Hindi scenes (boomerang bg) ==",flush=True)
subprocess.run(["python3","render_ep02_manim_hi.py"],cwd=HOME)
...
subprocess.run(["python3","focus_audio.py","--dur",str(int(float(d))),"--beat","12","--out","/tmp/bed_ep02_hi.wav"],cwd=HOME)
FOC=f"{HOME}/youtube_series/ep02_hi_FINAL_focus.mp4"
subprocess.run(["python3","focus_mix.py",OUT,"/tmp/bed_ep02_hi.wav",FOC],cwd=HOME)
print(f"\n✅ EP02_HI_DONE: {FOC}",flush=True)
