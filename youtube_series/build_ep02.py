#!/usr/bin/env python3
"""Full Episode 2 (EN): Mode-C scenes (boomerang bg) -> kinetic captions -> concat -> focus bed."""
import subprocess, os
HOME="/home/ubuntu"; B=f"{HOME}/youtube_series/ep02_manim_build"; BUILD=f"{HOME}/youtube_series/ep02_build"
OUT=f"{HOME}/youtube_series/ep02_FINAL.mp4"; N=13

print("== STEP 1: render scenes (boomerang bg) ==",flush=True)
subprocess.run(["python3","render_ep02_manim.py"],cwd=HOME)
...
subprocess.run(["python3","focus_audio.py","--dur",str(int(float(d))),"--beat","12","--out","/tmp/bed_ep02.wav"],cwd=HOME)
FOC=f"{HOME}/youtube_series/ep02_FINAL_focus.mp4"
subprocess.run(["python3","focus_mix.py",OUT,"/tmp/bed_ep02.wav",FOC],cwd=HOME)
print(f"\n✅ EP02_DONE: {FOC}",flush=True)
