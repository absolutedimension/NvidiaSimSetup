#!/usr/bin/env python3
"""Full EN Episode 8 "The Neuron": Mode-C scenes (dark bg) -> kinetic captions -> concat -> focus bed."""
import subprocess, os
HOME="/home/ubuntu"; B=f"{HOME}/youtube_series/ep08_manim_build"; BUILD=f"{HOME}/youtube_series/ep08_build"
OUT=f"{HOME}/youtube_series/ep08_FINAL.mp4"; N=14

print("== STEP 1: render scenes (dark bg) ==",flush=True)
subprocess.run(["python3","render_ep08_manim.py"],cwd=f"{HOME}/youtube_series")
...
subprocess.run(["python3","focus_audio.py","--dur",str(int(float(d))),"--beat","12","--out","/tmp/bed_ep08_en.wav"],cwd=HOME)
FOC=f"{HOME}/youtube_series/ep08_FINAL_focus.mp4"
subprocess.run(["python3","focus_mix.py",OUT,"/tmp/bed_ep08_en.wav",FOC],cwd=HOME)
print(f"\n✅ EP08_EN_DONE: {FOC}",flush=True)
