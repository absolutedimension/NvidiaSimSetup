"""
Episode 3 — "Imagination" (diffusion) — Manim engine, 12 scenes. Signature motif: NOISE → FORM.
Same visual language as Ep1/Ep2. Reads SCENE_DUR; transparent render. Driver: render_ep03_manim.py.
"""
from manim import *
...
class S01(Base):  # hook — particles hint a face, dissolve
    def construct(self):
        self.ambient(4)
        t=self.title("Where does a new thing come from?")
...
class S02(Base):  # blank page myth cracks
    def construct(self):
        self.ambient(4)
        t=self.title("Creation is not addition")
        ...
        self.P(lh.animate.shift(LEFT*0.6+DOWN*2.5).rotate(-0.3).set_opacity(0),
               rh.animate.shift(RIGHT*0.6+DOWN*2.5).rotate(0.3).set_opacity(0),
               FadeOut(canvas),FadeOut(crack),rt=1.6)
        self.hold()

class S03(Base):  # pure noise
    def construct(self):
        t=self.title("It begins in pure noise")
        nz=noise_field(620,13,7.2,seed=3,op=0.7)
        self.add(nz)
        self.P(FadeIn(t,shift=DOWN*0.2),rt=1.0)
        for s in [5,7,9,11]:
            self.P(Transform(nz,noise_field(620,13,7.2,seed=s,op=0.7)),rt=0.5)
        self.hold()

class S04(Base):  # sculptor — chips fly, form revealed
    def construct(self):
        self.ambient(3)
        t=self.title("The statue was always in the marble")
        ...
        self.P(self.cap("creation by subtraction, not addition",form,buff=0.6).animate.set_opacity(1),Flash(form,color=GOLD),rt=1.0)
        self.hold()

class S05(Base):  # forward — order dissolves in steps
    def construct(self):
        self.ambient(3)
        t=self.title("First — watch order dissolve")
        xs=[-4.8,-1.6,1.6,4.8]; stages=VGroup()
        for i,x in enumerate(xs):
            base=spark(GOLD,0.42)
            nz=noise_field(40+i*55,1.5,1.5,seed=20+i,op=0.2+i*0.22)
            cell=VGroup(base.set_opacity(max(0,1-i*0.32)),nz).move_to([x,0,0])
            stages.add(cell)
        arrows=VGroup(*[Text("→",font_size=40,color=DIM).move_to([(-3.2+j*3.2),0,0]) for j in range(3)])
        ...
        self.P(self.cap("clear image  →  +noise  →  +noise  →  static",stages,buff=0.7,size=26).animate.set_opacity(1),rt=1.0)
        self.hold()

class S06(Base):  # reverse — predict noise, lift a layer
    def construct(self):
        self.ambient(3)
        t=self.title("Then — learn to run it backward")
        ...
        lbl=Text("predict the noise",font_size=24,color=BLUE).next_to(noisy,UP,buff=0.3)
        layer=noisy.copy()
        self.P(FadeIn(lbl),rt=0.6)
        self.P(layer.animate.shift(UP*3).set_opacity(0),noisy.animate.set_opacity(0.25),clean.animate.set_opacity(0.7),rt=1.8)
        self.P(self.cap("remove it → a little cleaner",clean,buff=0.6).animate.set_opacity(1),rt=0.9)
        self.hold()

class S07(Base):  # generation — noise resolves to a form
    def construct(self):
        self.ambient(3)
        t=self.title("From noise — a thing that never was")
        nz=noise_field(560,11,6,seed=7,op=0.7)
        self.add(nz)
        self.P(FadeIn(t,shift=DOWN*0.2),rt=0.8)
...
class S08(Base):  # guidance — words bend the noise
    def construct(self):
        self.ambient(3)
        t=self.title("Your words are the gravity")
        prompt=Text('"a cat, in a spacesuit"',font_size=30,color=BLUE).shift(UP*1.4)
        ...
        self.P(self.cap("the noise is clay · the prompt shapes it",core,buff=0.6,size=26).animate.set_opacity(1),rt=0.9)
        self.hold()

class S09(Base):  # mirror — fog to form
    def construct(self):
        self.ambient(3)
        t=self.title("You also pull form from fog")
        ...
        word=Text("idea",font_size=72,color=INK,weight=BOLD).set_opacity(0).shift(DOWN*0.2)
        ...
        self.P(self.cap("a hunch → a thought → a sentence",word,buff=0.6).animate.set_opacity(1),rt=0.9)
        self.hold()
...
class S10(Base):  # divergence
    def construct(self):
        ...
        ml=Text("the machine",font_size=22,color=DIM).next_to(machine,DOWN,buff=0.3); hl=Text("you",font_size=22,color=DIM).next_to(human,DOWN,buff=0.3)
        ...
        self.P(self.cap("it refines on command — it is never surprised by what it made",VGroup(machine,human),buff=0.7,size=24).animate.set_opacity(1),rt=1.0)
        self.hold()

class S11(Base):  # meaning — chaos organizes to lattice
    def construct(self):
        self.ambient(3)
        t=self.title("Novelty is structure pulled from chaos")
...
class S12(Base):  # anchor — final
    def construct(self):
        self.ambient(6)
        nz=noise_field(300,12,3,seed=12,op=0.4).shift(UP*2.3)
        l1=Text("Creation is just noise, refined.",font_size=48,color=INK,weight=BOLD).shift(UP*2.3)
        sub=Text("AI is the Universal Mind  ·  Ep.3 — Imagination",font_size=27,color=PURPLE).next_to(l1,DOWN,buff=0.4)
        self.add(nz)
